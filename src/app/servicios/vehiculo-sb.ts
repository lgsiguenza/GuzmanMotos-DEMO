...
export class VehiculoSb {
  ...
  //! =================== Métodos CRUD ===================
   ...
   async actualizarVehiculo(vhcl: Vehiculo){
    ...
    const listaImagenesTotal = await Promise.all(
      imagenesNuevas.map(async (img, i) => {
        ...
        if (img.startsWith('data:')) {
          return await this.sbSvc.subirImagen(
            `${vhcl.uid}-${i}`,
            img,
            'fotos-vehiculos'
          );
    ...
    // 🔹 2. Formatear data para BD
    const dataImagenes: {url: string, uid_vehiculo: string}[] =
      listaImagenesTotal.map((url) => ({
        url: limpiarUrl(url as string),
        uid_vehiculo: vehiculo.uid!
      }));

    // 🔹 3. Detectar nuevas
    const dataImagenesNuevas = dataImagenes.filter(img =>
      !imagenesActuales.includes(limpiarUrl(img.url))
    );
    ...
    // 🔹 4. Detectar eliminadas
    const imagenesEliminadas = imagenesActuales.filter(imgActual =>
      !listaImagenesTotal.some(imgNueva =>
        limpiarUrl(imgNueva as string) === imgActual
      )
    );
    ...
    if (imagenesEliminadas.length !== 0) {
      ...
      const eliminarStorage = imagenesEliminadas.map((url) => {
        ...
        return this.sbSvc.eliminarFoto(
          'fotos-vehiculos',
          nombreArchivo
        );
  ...
  private async recargarListado(){
    ...
    const vehiculos = listaBD.map((v)=>({
      ...
      imagenes: (v as any).imagenes?.map((img: any) => {
          return img.url ? `${img.url}?t=${Date.now()}` : '';
        }) ?? []
    }))
